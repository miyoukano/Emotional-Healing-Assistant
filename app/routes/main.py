...
@main_bp.route('/create_default_avatar')
def create_default_avatar():
    ...
    try:
        ...
        # 复制默认头像
        default_avatar_path = os.path.join(img_dir, 'default_avatar.jpg')
        
        # 如果默认头像不存在，创建一个简单的默认头像
        if not os.path.exists(default_avatar_path):
            # 复制一个已有的图片作为默认头像
            source_path = os.path.join(current_app.root_path, 'static', 'img', 'logo.png')
            if os.path.exists(source_path):
                shutil.copy(source_path, default_avatar_path)
            else:
                # 如果没有可用的图片，创建一个空文件
